...
function CTP_Grid(CTPGrid::Array{Float64,2})  # may add plotting feature later - spacings paramter for here
    #print("Checking grid is changing",CTPGrid[4,6])
    # Labels for plotting
...
@doc """
    CTP_Grid_Part4(CTPGrid::Array{Float64,2})

    Returns tmm - minimum measured thickness determined at the time of the inspection
    Returns longitudinal and circumferential CTP
"""
function CTP_Grid_Part4(CTPGrid::Array{Float64,2})  # may add plotting feature later - spacings paramter for here
    #print("Checking grid is changing",CTPGrid[4,6])
    # Labels for plotting
    #=circumferential_plane_index = Int64.(collect(1:1:s2)) # collect auto rounds - no need to write code to correct
    circ_label = fill("C",length(circumferential_plane_index))
    ...
    minL_CTP = minimum(Longitudinal_CTP)
    tmm = minimum([minC_CTP,minL_CTP]) # minimum measured thickness determined at the time of the inspection.
        return tmm, Longitudinal_CTP, Circumferential_CTP
end
